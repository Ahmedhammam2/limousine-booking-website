    ...
    const carPerformance = await Booking.aggregate([
      {
        $match: {
          ...
          status: { $in: ["pending", "confirmed", "no show"] }, // Both count toward utilization
        },
      },
      {
        $addFields: {
          ...
          // Inclusive revenue amount (paid, pending, unpaid all count for projection)
          revenueAmount: {
            $sum: { $convert: { input: "$finalPrice", to: "double", onError: 0, onNull: 0 } }
          },
        },
      },
      {
