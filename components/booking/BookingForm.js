...
function BookingFormInner({ isEditing, initialData, bookingId }) {
  ...
  useEffect(() => {
    ...
    // If we have no search params and it's not a server-side initial data load,
    // we assume it's a fresh booking start and clear old customer session data.
    if (!searchParams.toString() && !initialData && typeof window !== "undefined") {
      sessionStorage.removeItem("limo_customer_data");
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare query string for both current URL replacement and navigation
    const query = new URLSearchParams(bookingData).toString();

    // UPDATE: Persist current form state into the history entry for the booking page.
    // This ensures that hitting "Back" from the next page restores the data properly.
