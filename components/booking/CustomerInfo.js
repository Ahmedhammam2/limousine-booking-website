...
export default function CustomerInfo({
  ...
  useEffect(() => {
    ...
    if (isEditing && editedBookingId) {
      ...
      // Otherwise, restore from sessionStorage if available
      const savedData = sessionStorage.getItem("limo_customer_data");
      if (savedData) {
        try {
  ...
  // Persist to sessionStorage whenever fields change
  useEffect(() => {
    if (isEditing) return; // Don't overwrite if we're in edit mode
    const data = JSON.stringify({
      name: customerName,
      email: customerEmail,
      phone: customerPhone,
    });
    sessionStorage.setItem("limo_customer_data", data);
  }, [customerName, customerEmail, customerPhone, isEditing]);
